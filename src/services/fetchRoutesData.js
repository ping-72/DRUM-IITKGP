...
async function getMapboxRoutes(source, destination) {
  console.log('Calling Mapbox API... in the fetchRoutesData.js file');
  ...
  console.log('Data got from Mapbox API', json);
  console.log('Routes got from Mapbox API', routes);
...
async function getGraphhopperRoutes(temp_mode, source, destination) {
  console.log('Calling Graphhopper API... in the fetchRoutesData.js file');
  const query = await fetch(
  ...
  console.log('Data got from Graphhopper API', json);
  console.log('Routes got from Graphhopper API', routes);
  return routes;
}
